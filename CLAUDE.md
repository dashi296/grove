...
**デスクトップ例外**: TanStack Router がファイルベースルーティングで `pages/` の役割を担うため、`apps/desktop` に `pages/` ディレクトリは置かない。TanStack Start は使用しない。ルートコンポーネントは `app/routes/` 以下に TanStack Router の規約に従って配置する。
