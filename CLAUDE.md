...
現時点のリポジトリは設計ドキュメント中心です。以下の構成・コマンド・規約は、実装着手後に従うべきターゲットアーキテクチャとして扱ってください。
...
想定ツール: **Turborepo** + **pnpm workspaces**
...
まだリポジトリに実装がない段階では、これらのコマンドが未定義でも問題ありません。実装を追加する際は、この命名に合わせて整備してください。
